# System on Chip

---

## HC-SR04 Ultrasonic

### Description  
This subfolder contains resources related to the **HC-SR04 Ultrasonic** sensor, including project files for **Vivado** and **Vitis**, as well as a block design PDF and a demonstration video.

### Contents  
- 📄 [PDF Block Design](./HC-SR04/design_1.pdf)  
- 💻 [Vitis Classic Project](./HC-SR04/vitis/)  
- 🛠️ [Vivado Project](./HC-SR04/vivado/)  
- ▶️ [YouTube Demo](https://youtu.be/epBwFla-cHY)

---

## Speed Sensor and Motor Control

### Description  
This subfolder contains resources related to the **L9110S motor driver** and the **LM393 speed sensor**, including project files for **Vivado** and **Vitis**, a block design PDF, and demonstration videos.

### Contents  
- 📄 [PDF Block Design](./MOTOR_DRIVER/design_2.pdf)  
- 💻 [Vitis Classic Project](./MOTOR_DRIVER/vitis/)  
- 🛠️ [Vivado Project](./MOTOR_DRIVER/vivado/)  
- ▶️ [YouTube Demo](https://www.youtube.com/watch?v=4ucw9PQOtl8&ab_channel=MauroDeBruyn)  
- ▶️ [YouTube Demo (Ultrasonic)](https://youtu.be/epBwFla-cHY)

---

## IMU

### Description  
This subfolder contains resources related to the **IMU**, including project files for **Vivado** and **Vitis**, as well as a block design PDF.

### Contents  
- 📄 [PDF Block Design](./IMU/design_1.pdf)  
- 💻 [Vitis Classic Project](./IMU/Vitis/)  
- 🛠️ [Vivado Project](./IMU/vivado/)

---

## Full Combined Project

### Description  
This subfolder contains resources related to the **full car project**, which combines all components above into a complete embedded system. Includes project files for **Vivado** and **Vitis**, a block design PDF, and a demonstration video.

### Contents  
- 📄 [PDF Block Design](https://github.com/OliwierJaworski/SOC/blob/main/Car_Project/blokdesign_car.pdf)  
- 💻 [Vitis Classic Project](./Car_Project/vitis/)  
- 🛠️ [Vivado Project](./Car_Project/vivado/)  
- ▶️ [YouTube Demo](https://youtu.be/AtfHGin1JEI?si=Db_pDSz_8XypDvtc)  

📘 More information about the full car system can be found in the [README file](./Car_Project/README.md).
